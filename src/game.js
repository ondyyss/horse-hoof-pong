/**
 * HOOF PONG - COMPLETE EDITION
 * - Přímé ovládání (Direct Swipe)
 * - Nápis kola na začátku
 * - Statistiky % úspěšnosti
 * - Tlačítko MENU s potvrzením
 * - Obrazovka VÍTĚZSTVÍ po vyházení všech kelímků
 */
const config = {
    type: Phaser.AUTO,
...
class GameScene extends Phaser.Scene {
    ...
    showExitConfirm() {
        ...
        this.dialog.add([panel, title, yesBtn, noBtn, 
            this.add.text(-70, 40, 'ANO', {fontSize: '18px', color: '#fff', fontStyle: 'bold'}).setOrigin(0.5), 
            this.add.text(70, 40, 'NE', {fontSize: '18px', color: '#fff', fontStyle: 'bold'}).setOrigin(0.5)]);
    ...
    showVictory() {
        ...
        const title = this.add.text(0, -100, 'VÍTĚZSTVÍ!', { fontSize: '60px', fill: '#f1c40f', fontStyle: '900' }).setOrigin(0.5);
        const stats = this.add.text(0, 0, `Trefeno kelímků: ${this.totalHits}\nPočet hodů: ${this.totalShots}\nÚspěšnost: ${acc}%`, { 
            fontSize: '26px', fill: '#fff', align: 'center', lineSpacing: 12 
        }).setOrigin(0.5);

        const restartBtn = this.add.rectangle(0, 140, 220, 60, 0x27ae60).setInteractive();
        this.add.text(0, 140, 'HRÁT ZNOVU', { fontSize: '24px', fill: '#fff', fontStyle: 'bold' }).setOrigin(0.5);
        
        panel.add([title, stats, restartBtn]);
    ...
    processTurn() {
        ...
        if (this.shotsInRound >= 2) {
            this.currentRound++; this.shotsInRound = 0;
            this.showRoundIntro();
        }
        this.updateUI(); 
        this.resetBall();
    ...
    spawnCups() {
        ...
        let layout = [4, 3, 2, 1]; // Celkem 10 kelímků
